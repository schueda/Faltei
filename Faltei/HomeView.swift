...
struct HomeView: View {
    let periodo: Periodo? = Periodo(name: "2021/1", endDate: Date(), materias: [Materia(name: "Introdução a Computação Científica", maxFaltas: 14, color: .blue, faltas: []), Materia(name: "Calculo2", maxFaltas: 14, color: .yellow, faltas: [
            Falta(quantidade: 2, date: Calendar.current.date(byAdding: DateComponents(day: -10), to:  Date())!),
            Falta(quantidade: 2, date: Calendar.current.date(byAdding: DateComponents(day: -40), to:  Date())!),
            Falta(quantidade: 2, date: Calendar.current.date(byAdding: DateComponents(day: -60), to: Date())!),
            Falta(quantidade: 2, date: Calendar.current.date(byAdding: DateComponents(day: -2), to: Date())!),
                 ])])
...
//    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showNewMateria) {
